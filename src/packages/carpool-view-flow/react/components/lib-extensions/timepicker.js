...
Timepicker.Clock = React.createClass({
    ...
    renderHoursBubbles: function () {
        ...
        for (; index < positions.length; ++index) {
            ...
            bubbles.push(React.createElement(
                ...
                React.createElement('circle', { cx: x, cy: y, fill: '#79c7ff', r: 15 }),
                React.createElement(
                    'text',
                    { x: x, y: y },
                    hour
    ...
    onClickSvg: function (e, a, b) {
      const clientRect = e.currentTarget.getBoundingClientRect()
      const cx = (clientRect.right + clientRect.left) / 2
      const cy = (clientRect.bottom + clientRect.top) / 2

      const xOff = e.pageX - cx
      const yOff = e.pageY - cy
      const angle = Math.atan2(yOff, xOff)

      if (this.state.mode === Timepicker.HOURS) {
        // Hours selection
        const hours12 = Math.round(((((angle / Math.PI) + 1) * 6) + 9) % 12)
        if (this.props.militaryTime) {
          let hours24 = hours12
          const clickR = Math.sqrt(Math.pow(xOff, 2) + Math.pow(yOff, 2))
    ...
    onClickHour: function (hours) {
      ...
      return function (event, preventChangeMode) {
        return // Disabled cause of onClickSvg implementation
